...
export async function getStaticProps(context) {
  ...
  const resonse = await axios
    .get(
      `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${APIKey}&lang=vi&units=metric`
    )
    .then(res => res)
        ...
        </div>
        {
          <div className={css['container__weather']}>
            ...
            <div className={css['container__weather__detail']}>
              <p className={css['container__weather__detail__description']}>
                {weatherData.weather[0].description}
              </p>
              <p className={css['container__weather__detail__humidity']}>
                Humidity: {weatherData.main.humidity}%
              </p>
              <p className={css['container__weather__detail__wind']}>
