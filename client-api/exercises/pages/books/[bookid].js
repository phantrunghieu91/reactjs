...
export default function BookDetails() {
  ...
  const { bookid } = router.query; //useParams();
  ...
  const handleChange = e => {
    ...
    if (inputName === 'quantity') {
      if (!book.quantity) {
        ...
        setErrors({
          ...errors,
          [inputName]: /[0-9]{1,}/.test(inputValue) ? '' : 'Only number!',
        });
      }
    } else
      setErrors({
        ...errors,
        [inputName]: /[A-Za-z0-9\s]{1,}/.test(inputValue) ? '' : 'Required',
      });
    ...
    if (isError || !isFilled) {
      ...
      if (isAddNew) {
        client
          ...
          .then(res => {
            alert(
              `Successful create new book: ${JSON.stringify(res.data, null, 2)}`
            );
          })
          .catch(err => {
            throw err;
